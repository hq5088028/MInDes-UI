...
class FontStyleEditor(QGroupBox):
    ...
    def load(self, style: TextStyle):
        ...
        self.font_combo.setCurrentFont(
            self.font_combo.currentFont().__class__(style.font)
        )
...
class PlotPropertyDialog(QDialog):
    def __init__(
        ...
        columns: list[str],
        apply_callback,
        save_format_callback,
        parent=None,
        *,
        shared_y_axis=False,
        curve_names=None,
        curve_columns=None,
        initial_curve_id=None,
    ...
    def _build_overall(self):
        ...
        self.unit_combo.addItems(["cm", "in", "mm"])
    ...
    def _build_axes(self):
        ...
        self.tick_direction.addItems(["in", "out", "inout"])
        self.major_length = self._double(0, 30, 2)
        self.minor_length = self._double(0, 30, 2)
        ...
        for widget in (
            ...
            signal = (
                getattr(widget, "currentTextChanged", None)
                or getattr(widget, "toggled", None)
                or getattr(widget, "valueChanged", None)
                or getattr(widget, "textChanged")
            )
            signal.connect(self._save_axis)
        self.tick_color.colorChanged.connect(self._save_axis)
        self.spine_color.colorChanged.connect(self._save_axis)
    ...
    def _build_grid(self):
        ...
        self.grid_line.currentTextChanged.connect(self._save_grid)
        self.grid_width.valueChanged.connect(self._save_grid)
        self.grid_alpha.valueChanged.connect(self._save_grid)
    ...
    def _build_curves(self):
        ...
        self.marker_edge_width = self._double(0, 10, 2)
        ...
        self.error_every.setRange(1, 100000)
        self.capsize = self._double(0, 30, 2)
        self.capthick = self._double(0.1, 10, 2)
        self.error_width = self._double(0.1, 10, 2)
        ...
        self.up_btn.clicked.connect(lambda: self._move_curve(-1))
        self.down_btn.clicked.connect(lambda: self._move_curve(1))
        for widget in (
            self.curve_visible,
            self.legend_edit,
            self.line_width,
    ...
    def _curve_names(self):
        if self.shared_y_axis:
            return [
                self.curve_names.get(c.column, c.legend_text or c.column)
                for c in self.config.curves
            ]
        return [
            f"{'Left' if c.side == 'left' else 'Right'}: {self.curve_names.get(c.column, c.column)}"
            for c in self.config.curves
        ]
    ...
    def _load_all(self):
        ...
        for widget, value in (
            ...
            (self.margin_top, c.margin_top_cm),
            (self.margin_bottom, c.margin_bottom_cm),
        ):
            widget.setValue(convert_length(value, "cm", unit))
        self.title_editor.load(c.title)
        self.xlabel_editor.load(c.x_axis.label)
        self.latex_cb.setChecked(c.use_latex)
        self.top_spine_cb.setChecked(c.show_top_spine)
        self.bottom_spine_cb.setChecked(c.show_bottom_spine)
        self.left_spine_cb.setChecked(c.show_left_spine)
        self.right_spine_cb.setChecked(c.show_right_spine)
        ...
        if not self._initial_curve_applied and self.initial_curve_id:
            initial_index = next(
                (
                    index
                    for index, curve in enumerate(c.curves)
                    if curve.column == self.initial_curve_id
                ),
                -1,
            )
            if initial_index >= 0:
                self.curve_combo.setCurrentIndex(initial_index)
                self.text_curve_combo.setCurrentIndex(initial_index)
        ...
        self.legend_cols.setValue(legend.columns)
        self.frame_cb.setChecked(legend.frame_visible)
        self.frame_edge.set_color(legend.edge_color)
        self.frame_face.set_color(legend.face_color)
        self.frame_alpha.setValue(legend.frame_alpha)
        self.legend_font.load(legend.font)
        self.dpi_spin.setValue(c.export_dpi)
        self._loading = False
        self._load_curve_text()
        self._load_axis()
        self._load_grid()
        self._load_curve()

    def _unit_changed(self, unit):
        if self._loading or unit == self._last_unit:
            return
        from .plot_config import convert_length
    ...
    def _save_overall(self):
        ...
        c.show_right_spine = self.right_spine_cb.isChecked()
        c.margin_left_cm = convert_length(self.margin_left.value(), unit, "cm")
        c.margin_right_cm = convert_length(self.margin_right.value(), unit, "cm")
        c.margin_top_cm = convert_length(self.margin_top.value(), unit, "cm")
        c.margin_bottom_cm = convert_length(self.margin_bottom.value(), unit, "cm")
        self.title_editor.save(c.title)
        self.xlabel_editor.save(c.x_axis.label)
        ...
        legend.columns = self.legend_cols.value()
        legend.frame_visible = self.frame_cb.isChecked()
        legend.edge_color = self.frame_edge.color()
        legend.face_color = self.frame_face.color()
        legend.frame_alpha = self.frame_alpha.value()
        self.legend_font.save(legend.font)
    ...
    def _load_curve_text(self, *_):
        ...
        if self.shared_y_axis:
            ...
            self._loading = False
            return
        if self.text_curve_combo.currentIndex() < 0:
            return
        self._loading = True
        curve = self.config.curves[self.text_curve_combo.currentIndex()]
        self.ylabel_editor.load(curve.axis.label)
        self.ytick_editor.load(curve.axis.tick.font)
        self._loading = False
    ...
    def _save_curve_text(self):
        ...
        if self.text_curve_combo.currentIndex() < 0:
            return
        curve = self.config.curves[self.text_curve_combo.currentIndex()]
        self.ylabel_editor.save(curve.axis.label)
        self.ytick_editor.save(curve.axis.tick.font)
    ...
    def _load_axis(self, *_):
        ...
        self.invert_cb.setChecked(a.inverted)
        self.major_cb.setChecked(t.major_visible)
        self.minor_cb.setChecked(t.minor_visible)
        self.tick_direction.setCurrentText(t.direction)
        self.tick_bottom_cb.setChecked(t.show_bottom)
        self.tick_top_cb.setChecked(t.show_top)
        self.tick_left_cb.setChecked(t.show_left)
        self.tick_right_cb.setChecked(t.show_right)
        self.major_length.setValue(t.major_length)
        ...
        self.tick_step.setValue(t.step)
        self.spine_cb.setChecked(a.spine_visible)
        self.spine_width.setValue(a.spine_width)
        self.spine_color.set_color(a.spine_color)
        self.xtick_editor.load(t.font)
        self._loading = False
    ...
    def _save_axis(self, *_):
        ...
        a.inverted = self.invert_cb.isChecked()
        t.major_visible = self.major_cb.isChecked()
        t.minor_visible = self.minor_cb.isChecked()
        t.direction = self.tick_direction.currentText()
        t.major_length = self.major_length.value()
        t.minor_length = self.minor_length.value()
        t.width = self.tick_width.value()
        t.color = self.tick_color.color()
        t.show_bottom = self.tick_bottom_cb.isChecked()
        ...
        t.step = self.tick_step.value()
        self.xtick_editor.save(t.font)
        a.spine_visible = self.spine_cb.isChecked()
        a.spine_width = self.spine_width.value()
        a.spine_color = self.spine_color.color()

    def _load_grid(self, *_):
        if self._loading:
            return
        self._loading = True
        g = self._axis_style(self.grid_axis_combo).grid
        self.grid_cb.setChecked(g.visible)
        self.grid_color.set_color(g.color)
        self.grid_line.setCurrentText(g.linestyle)
        self.grid_width.setValue(g.linewidth)
        self.grid_alpha.setValue(g.alpha)
        self._loading = False
    ...
    def _save_grid(self, *_):
        if self._loading:
            return
        g = self._axis_style(self.grid_axis_combo).grid
        g.visible = self.grid_cb.isChecked()
        g.color = self.grid_color.color()
        g.linestyle = self.grid_line.currentText()
        g.linewidth = self.grid_width.value()
        g.alpha = self.grid_alpha.value()

    def _load_curve(self, *_):
        if self._loading or self.curve_combo.currentIndex() < 0:
            return
        self._loading = True
        ...
        self.error_column.clear()
        self.error_column.addItems(available)
        self.curve_visible.setChecked(c.visible)
        ...
        self.capsize.setValue(e.capsize)
        self.capthick.setValue(e.capthick)
        self.error_width.setValue(e.linewidth)
        self.error_color.set_color(e.color)
        self.band_color.set_color(e.fill_color)
        self.band_alpha.setValue(e.fill_alpha)
        self._loading = False
    ...
    def _save_curve(self, *_):
        ...
        c = self.config.curves[self.curve_combo.currentIndex()]
        e = c.error
        c.visible = self.curve_visible.isChecked()
        ...
        e.capsize = self.capsize.value()
        e.capthick = self.capthick.value()
        e.linewidth = self.error_width.value()
        e.color = self.error_color.color()
        e.fill_color = self.band_color.color()
        e.fill_alpha = self.band_alpha.value()

    def _move_curve(self, delta):
        idx = self.curve_combo.currentIndex()
        new = idx + delta
        if idx < 0 or new < 0 or new >= len(self.config.curves):
            return
        if self.config.curves[idx].side != self.config.curves[new].side:
            return
        self.config.curves[idx], self.config.curves[new] = (
            self.config.curves[new],
            self.config.curves[idx],
        )
        self._load_all()
        self.curve_combo.setCurrentIndex(new)
        self._load_curve()

    def _validate(self):
        self._save_overall()
        self._save_curve_text()
        self._save_axis()
        self._save_grid()
        self._save_curve()
        c = self.config
        if (
            c.width_cm <= c.margin_left_cm + c.margin_right_cm
    ...
    def _apply(self):
        if self._validate():
            self._apply_callback(self.config.copy())
    ...
    def _load_format(self):
        if self._load_format_callback is None or not self._validate():
            return
        loaded = self._load_format_callback(
            self.config.copy(), list(self.format_templates)
        )
        if loaded is None:
            return
    ...
    def _restore_factory(self):
        ...
        for i, (column, side) in enumerate(old):
            self.config.curves.append(new_curve(column, side, i))
        self._last_unit = self.config.unit
        self._load_all()
