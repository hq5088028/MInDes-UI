...
class CubeAxesBundle:
    ...
    def actors(self):
        return [
            self.base,
            *(self.labels[name] for name in "XYZ"),
            *(self.titles[name] for name in "XYZ"),
        ]


def _apply_text_style(prop, style):
    family = {
        "Arial": prop.SetFontFamilyToArial,
        "Courier": prop.SetFontFamilyToCourier,
        "Times": prop.SetFontFamilyToTimes,
    }.get(style.font, prop.SetFontFamilyToArial)
    family()
    prop.SetFontSize(int(style.size))
    prop.SetBold(bool(style.bold))
    prop.SetItalic(bool(style.italic))
    prop.SetShadow(False)
    prop.SetColor(*hex_to_rgb(style.color))
    prop.Modified()
...
def _configure_cube_common(actor, config, bounds, raw_min, raw_max, camera=None):
    ...
    actor.SetTickLocation(
        {
            "Inside": actor.VTK_TICKS_INSIDE,
            "Outside": actor.VTK_TICKS_OUTSIDE,
            "Both": actor.VTK_TICKS_BOTH,
        }.get(config.tick_location, actor.VTK_TICKS_INSIDE)
    )
    actor.SetFlyMode(
        {
            "Closest Triad": actor.VTK_FLY_CLOSEST_TRIAD,
            "Furthest Triad": actor.VTK_FLY_FURTHEST_TRIAD,
            "Outer Edges": actor.VTK_FLY_OUTER_EDGES,
            "Static Triad": actor.VTK_FLY_STATIC_TRIAD,
            "Static Edges": actor.VTK_FLY_STATIC_EDGES,
        }.get(config.fly_mode, actor.VTK_FLY_CLOSEST_TRIAD)
    )
    actor.SetGridLineLocation(
        {
            "All": actor.VTK_GRID_LINES_ALL,
            "Closest": actor.VTK_GRID_LINES_CLOSEST,
            "Furthest": actor.VTK_GRID_LINES_FURTHEST,
        }.get(config.grid_line_location, actor.VTK_GRID_LINES_ALL)
    )
...
def configure_cube_axes(actor, config, bounds, raw_min, raw_max, camera=None):
    ...
    for index, (name, axis) in enumerate(
        zip("XYZ", (config.x_axis, config.y_axis, config.z_axis))
    ):
        getattr(actor, f"Set{name}Title")(
            axis.title if axis.title_visible and axis.axis_visible else ""
        )
        getattr(actor, f"Set{name}AxisVisibility")(bool(axis.axis_visible))
        getattr(actor, f"Set{name}AxisLabelVisibility")(
            bool(axis.axis_visible and axis.label_visible)
        )
        getattr(actor, f"Set{name}AxisTickVisibility")(
            bool(axis.axis_visible and axis.major_tick_visible)
        )
        getattr(actor, f"Set{name}AxisMinorTickVisibility")(
            bool(axis.axis_visible and axis.minor_tick_visible)
        )
        getattr(actor, f"SetDraw{name}Gridlines")(
            bool(axis.axis_visible and axis.grid_visible)
        )
...
def _text_layer(config, bounds, raw_min, raw_max, camera, selected, kind):
    ...
    for index, (name, axis) in enumerate(
        ...
        selected_visible = (
            name == selected
            and axis.axis_visible
            and (axis.title_visible if kind == "title" else axis.label_visible)
        )
...
def build_cube_axes_bundle(config, bounds, raw_min, raw_max, camera=None):
    ...
    base.Modified()
    labels = {
        name: _text_layer(config, bounds, raw_min, raw_max, camera, name, "label")
        for name in "XYZ"
    }
    titles = {
        name: _text_layer(config, bounds, raw_min, raw_max, camera, name, "title")
        for name in "XYZ"
    }
...
def aggregate_xy(x: np.ndarray, y: np.ndarray, z: np.ndarray):
    ...
    for (xv, yv), values in buckets.items():
        ...
        xs.append(xv)
        ys.append(yv)
        zs.append(float(np.mean(finite)) if finite else np.nan)
...
def build_surface_with_holes(x, y, z, transform=None) -> SurfaceBuildResult:
    ...
    if len(x) < 3 or finite_z.sum() < 3:
        return SurfaceBuildResult(
            None, "fewer than three valid points", int(finite_z.sum())
        )
    ...
    validity = vtk.vtkUnsignedCharArray()
    validity.SetName("csv_valid_z")
    raw_z = vtk.vtkDoubleArray()
    raw_z.SetName("csv_z")
    ...
    source.GetPointData().AddArray(validity)
    source.GetPointData().SetScalars(raw_z)
    ...
    if out_validity is None:
        return SurfaceBuildResult(
            None, "triangulation lost validity metadata", int(finite_z.sum())
        )

    kept = vtk.vtkCellArray()
    ids = vtk.vtkIdList()
    ...
    if kept.GetNumberOfCells() == 0:
        return SurfaceBuildResult(
            None, "no valid triangle remains after applying holes", int(finite_z.sum())
        )
    result = vtk.vtkPolyData()
    result.SetPoints(output.GetPoints())
    result.SetPolys(kept)
...
def build_scatter(x, y, z, transform=None) -> vtk.vtkPolyData:
    ...
    for point, zv in zip(display, z[valid]):
        pid = points.InsertNextPoint(*map(float, point))
    ...
    poly.GetPointData().SetScalars(raw_z)
    return poly
...
def make_lookup_table(name: str, value_range: tuple[float, float]):
    ...
    cmap_name = {"grayscale": "gray", "cool-warm": "coolwarm"}.get(
        name.lower(), name.lower()
    )
    cmap = matplotlib.colormaps.get_cmap(
        cmap_name if cmap_name in matplotlib.colormaps else "viridis"
    )
