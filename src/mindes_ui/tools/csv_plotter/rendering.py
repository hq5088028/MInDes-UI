...
def apply_locator_formatter(axis, style):
    ...
    if tick.manual_mode == "Positions":
        values = [
            float(value.strip()) for value in tick.positions.split(",") if value.strip()
        ]
        if values:
        ...
        axis.set_major_locator(
            ticker.FixedLocator(tick.start + np.arange(max(0, count)) * tick.step)
        )
    ...
    if tick.format_mode == "Fixed":
        ...
        axis.set_major_formatter(
            ticker.PercentFormatter(xmax=1.0, decimals=tick.decimals)
        )


def apply_scale(ax, style, dimension):
    scale = {"Linear": "linear", "Log10": "log", "SymLog": "symlog"}.get(
        style.scale, "linear"
    )
...
def render_shared_figure(figure, config, series):
    ...
    figure.set_size_inches(
        config.width_cm / 2.54, config.height_cm / 2.54, forward=False
    )
    left, right = (
        config.margin_left_cm / config.width_cm,
        1 - config.margin_right_cm / config.width_cm,
    )
    bottom, top = (
        config.margin_bottom_cm / config.height_cm,
        1 - config.margin_top_cm / config.height_cm,
    )
    ...
    apply_scale(ax, xstyle, "x")
    apply_scale(ax, ystyle, "y")
    ...
    for item in series:
        ...
        (line,) = ax.plot(
            ...
            markeredgewidth=curve.marker_edge_width,
            markevery=max(1, curve.markevery),
            label=curve.legend_text or item["label"],
        )
        handles.append(line)
        labels.append(curve.legend_text or item["label"])
        error = curve.error
        error_values = (
            item.get("errors", {}).get(error.column)
        ...
        if error.mode != "None" and error_values is not None:
            ...
            if error.mode in ("Bars", "Bars + Band"):
                ax.errorbar(
                    ...
                    yerr=err,
                    fmt="none",
                    errorevery=max(1, error.every),
                    ecolor=error.color,
                    elinewidth=error.linewidth,
                    capsize=error.capsize,
                    capthick=error.capthick,
                )
            if error.mode in ("Band", "Bars + Band"):
                ax.fill_between(
                    ...
                    y - err,
                    y + err,
                    color=error.fill_color,
                    alpha=error.fill_alpha,
                    linewidth=0,
                )

    if config.title.visible and config.title.text:
        text = ax.set_title(config.title.text, **font_kwargs(config.title))
        text.set_usetex(latex)
    if xstyle.label.visible:
        text = ax.set_xlabel(xstyle.label.text or "X", **font_kwargs(xstyle.label))
        text.set_usetex(latex)
    if ystyle.label.visible:
        text = ax.set_ylabel(ystyle.label.text or "Y", **font_kwargs(ystyle.label))
        text.set_usetex(latex)
    for dimension, style in (("x", xstyle), ("y", ystyle)):
        ...
        if dimension == "x":
            ax.tick_params(
                axis="x",
                bottom=tick.major_visible and tick.show_bottom,
            ...
            ax.tick_params(
                axis="x",
                which="minor",
                bottom=tick.minor_visible and tick.show_bottom,
                top=tick.minor_visible and tick.show_top,
                direction=tick.direction,
                length=tick.minor_length,
                width=tick.width,
                colors=tick.color,
            )
            ...
            ax.tick_params(
                axis="y",
                left=tick.major_visible and tick.show_left,
                right=tick.major_visible and tick.show_right,
                **kwargs,
            )
            ax.tick_params(
                axis="y",
                which="minor",
                left=tick.minor_visible and tick.show_left,
                right=tick.minor_visible and tick.show_right,
                direction=tick.direction,
                length=tick.minor_length,
                width=tick.width,
                colors=tick.color,
        ...
        for label in labels_to_style:
            label.update(font_kwargs(tick.font))
            label.set_usetex(latex)
        grid = style.grid
        ax.grid(
            grid.visible,
            axis=dimension,
    ...
    for name, visible in (
        ("top", config.show_top_spine),
        ("bottom", config.show_bottom_spine),
        ("left", config.show_left_spine),
        ("right", config.show_right_spine),
    ):
    ...
    if config.legend.visible and handles:
        kwargs: dict[str, Any] = dict(
            ...
            facecolor=config.legend.face_color,
            edgecolor=config.legend.edge_color,
            framealpha=config.legend.frame_alpha,
            prop={
                "family": config.legend.font.font,
                "size": config.legend.font.size,
        ...
        for text in legend.get_texts():
            text.set_color(config.legend.font.color)
            text.set_usetex(latex)
    return ax
