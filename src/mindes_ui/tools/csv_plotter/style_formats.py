...
def _reset_axis_data(axis: AxisStyle) -> None:
    axis.label.text = ""
    axis.auto_range = True
    axis.minimum = 0.0
    axis.maximum = 1.0
    tick = axis.tick
    tick.manual_mode = "Auto"
    tick.positions = ""
    tick.start = 0.0
    tick.stop = 1.0
    tick.step = 0.1
...
def _restore_axis_data(target: AxisStyle, source: AxisStyle) -> None:
    ...
    target.auto_range = source.auto_range
    target.minimum = source.minimum
    target.maximum = source.maximum
    target.tick.manual_mode = source.tick.manual_mode
    target.tick.positions = source.tick.positions
    target.tick.start = source.tick.start
    target.tick.stop = source.tick.stop
    target.tick.step = source.tick.step


def _strip_axis_data_dict(raw: dict) -> None:
    raw.get("label", {}).pop("text", None)
    for key in ("auto_range", "minimum", "maximum"):
        raw.pop(key, None)
    tick = raw.get("tick", {})
    for key in ("manual_mode", "positions", "start", "stop", "step"):
        tick.pop(key, None)


def _curve_template_dict(curve: CurveStyle) -> dict:
    raw = asdict(sanitize_curve_template(curve))
    for key in ("column", "legend_text", "side"):
        raw.pop(key, None)
    _strip_axis_data_dict(raw.get("axis", {}))
    error = raw.get("error", {})
    for key in ("mode", "source", "column", "constant"):
        error.pop(key, None)
    return raw
...
def sanitize_curve_template(curve: CurveStyle) -> CurveStyle:
    ...
    result.error.mode = "None"
    result.error.source = "Constant"
    result.error.column = ""
    result.error.constant = 0.0
    return result


def apply_curve_template(curve: CurveStyle, template: CurveStyle) -> CurveStyle:
    result = deepcopy(template)
    result.column = curve.column
    result.legend_text = curve.legend_text
    result.side = curve.side
    _restore_axis_data(result.axis, curve.axis)
    result.error.mode = curve.error.mode
    result.error.source = curve.error.source
    result.error.column = curve.error.column
    result.error.constant = curve.error.constant
    return result
...
def make_2d_style_payload(config: FigureConfig, templates=None) -> dict:
    ...
    clean.curves = []
    clean.title.text = ""
    _reset_axis_data(clean.x_axis)
    _reset_axis_data(clean.shared_y_axis)
    ...
    _strip_axis_data_dict(figure_raw.get("x_axis", {}))
    _strip_axis_data_dict(figure_raw.get("shared_y_axis", {}))
    return {
        "kind": STYLE_2D_KIND,
...
def parse_2d_style_payload(raw: dict) -> tuple[FigureConfig, list[CurveStyle]]:
    ...
    for index, value in enumerate(template_raw):
        _validate_dataclass_payload(CurveStyle(), value, f"curve_templates[{index}]")
    combined = deepcopy(figure_raw)
    combined["curves"] = template_raw
    parsed = FigureConfig.from_dict(combined)
    templates = [sanitize_curve_template(value) for value in parsed.curves]
    parsed.curves = []
    parsed.title.text = ""
    _reset_axis_data(parsed.x_axis)
    _reset_axis_data(parsed.shared_y_axis)
    return parsed, templates


def apply_2d_visual_style(
    current: FigureConfig, style: FigureConfig, templates: list[CurveStyle]
) -> FigureConfig:
    result = style.copy()
    result.title.text = current.title.text
    _restore_axis_data(result.x_axis, current.x_axis)
    _restore_axis_data(result.shared_y_axis, current.shared_y_axis)
    result.curves = []
    for index, curve in enumerate(current.curves):
        result.curves.append(
            apply_curve_template(curve, templates[index])
...
def apply_dataset_template(
    dataset: CsvDatasetConfig, template: CsvDatasetConfig
) -> CsvDatasetConfig:
    result = deepcopy(template)
    for name in (
        "dataset_id",
...
def parse_3d_style_payload(raw: dict) -> tuple[VtkPlotConfig, list[CsvDatasetConfig]]:
    if (
        ...
        or raw.get("version") not in (STYLE_VERSION, STYLE_3D_VERSION)
    ):
    ...
    config.z_max = 1.0
    templates = [
        sanitize_dataset_template(CsvDatasetConfig.from_dict(value))
