...
class CSVPlotterDialog(QDialog):
    ...
    def _set_size(self, parent):
        ...
        if parent is not None:
            geo = parent.geometry()
            width = max(1100, min(int(geo.width() * 0.95), screen.width() - 30))
            height = max(760, min(int(geo.height() * 0.95), screen.height() - 50))
            self.resize(width, height)
            self.move(
                max(screen.x(), geo.x() + (geo.width() - width) // 2),
                max(screen.y(), geo.y() + (geo.height() - height) // 2),
            )
        else:
            self.resize(
    ...
    def _load_state(self):
        ...
        return CsvPlotterState(
            splitter_sizes=[
                int(value) for value in sizes if isinstance(value, (int, float))
            ]
        )
    ...
    def _migrate_legacy_state(self):
        ...
        if not self.settings.contains(STYLE_3D_KEY):
            ...
            datasets = [
                ordered[value] for value in legacy.render_order_3d if value in ordered
            ]
            self.settings.setValue(
                STYLE_3D_KEY,
                json.dumps(
                    make_3d_style_payload(legacy.vtk, datasets), ensure_ascii=False
                ),
            )
        self.settings.remove(STATE_KEY)
        self.settings.remove("csv_plotter/default_2d_v1")
    ...
    def _persist_2d_visual_style(self, templates=None):
        templates = list(
            templates if templates is not None else self._curve_style_templates
        )
        self._curve_style_templates = [
            sanitize_curve_template(value) for value in templates
        ]
    ...
    def _persist_3d_visual_style(self, templates=None):
        ...
        self._dataset_style_templates = [
            sanitize_dataset_template(value) for value in templates
        ]
    ...
    def _build_ui(self):
        ...
        for text, slot in (
            ...
            data_buttons.addWidget(button, 1)
            self.data_action_buttons.append(button)
        data_minimum = max(
            button.minimumSizeHint().width() for button in self.data_action_buttons
        )
        for button in self.data_action_buttons:
            button.setMinimumWidth(data_minimum)
        right = QWidget()
    ...
    def _build_2d_tab(self):
        ...
        self.figure = Figure(figsize=(6.3, 3.94), dpi=100)
    ...
    def _build_3d_tab(self):
        ...
        self.axes_bundle = None
        self.scalarbar = vtk.vtkScalarBarActor()
        self.scalarbar.SetNumberOfLabels(5)
        self.scalarbar.SetPosition(0.86, 0.15)
    ...
    def _build_3d_action_buttons(self, layout):
        ...
        for text, slot, row, column in actions:
            ...
            self.view_action_buttons.append(button)
        view_minimum = max(
            button.minimumSizeHint().width() for button in self.view_action_buttons
        )
        for button in self.view_action_buttons:
            button.setMinimumWidth(view_minimum)
    ...
    def _restore_datasets(self):
        ...
        if self.state.active_dataset_id not in valid_ids:
            self.state.active_dataset_id = (
                self.state.datasets[0].dataset_id if self.state.datasets else ""
            )
        self._refresh_cards()
    ...
    def add_csv(self):
        ...
        for path in paths:
            ...
            self.frames[dataset.dataset_id] = frame
            self.state.render_order_2d.append(dataset.dataset_id)
            self.state.render_order_3d.append(dataset.dataset_id)
            self._append_card(dataset)
    ...
    def _append_card(self, dataset, index=None):
        ...
        card = DatasetControlCard(
            dataset, columns, frame is not None, self.cards_container
        )
        ...
        card.removeRequested.connect(
            lambda dataset_id: self.remove_dataset(dataset_id, True)
        )
        if index is None:
    ...
    def duplicate_dataset(self, dataset_id):
        ...
        source_curve = next(
            (
                curve
                for curve in self.figure_config.curves
                if curve.column == dataset_id
            ),
            None,
        )
    ...
    def _move_card(self, dataset_id, action):
        index = next(
            (i for i, card in enumerate(self.cards) if card.dataset_id == dataset_id),
            -1,
        )
        if index < 0:
            return
        targets = {
            "up": max(0, index - 1),
            "down": min(len(self.cards) - 1, index + 1),
            "top": 0,
        ...
        dataset = self.state.datasets.pop(index)
    ...
    def remove_selected(self):
        ...
        for item in list(selected):
            self.remove_dataset(item.dataset_id, False)
    ...
    def reload_selected(self):
        ...
        errors = [
            message
            for item in targets
            if (message := self.reload_dataset(item.dataset_id, False))
        ]
        if errors:
            QMessageBox.warning(
                self,
    ...
    def reload_dataset(self, dataset_id, show_error=True):
        ...
        card.update_columns([str(value) for value in frame.columns])
        self._save_state()
        return ""
    ...
    def remove_dataset(self, dataset_id, confirm=True):
        ...
        if (
            ...
            return
        index = self.state.datasets.index(dataset)
        card = self.cards.pop(index)
        self.state.datasets.pop(index)
        self.cards_layout.removeWidget(card)
        card.deleteLater()
        self.frames.pop(dataset_id, None)
        self.state.render_order_2d = [
            value for value in self.state.render_order_2d if value != dataset_id
        ]
        self.state.render_order_3d = [
            value for value in self.state.render_order_3d if value != dataset_id
        ]
        self.figure_config.curves = [
            curve for curve in self.figure_config.curves if curve.column != dataset_id
        ]
    ...
    def _sync_figure_curves(self):
        existing = {curve.column: curve for curve in self.figure_config.curves}
        curves = []
        ...
        for index, dataset_id in enumerate(order):
            ...
            if curve is None:
                curve = new_curve(dataset.dataset_id, "left", index)
                curve.color = dataset.color
                curve.marker_edge_color = dataset.color
                curve.legend_text = dataset_display_name(dataset)
                if index < len(self._curve_style_templates):
                    curve = apply_curve_template(
                        curve, self._curve_style_templates[index]
                    )
            curves.append(curve)
    ...
    def _series_2d(self):
        ...
        for dataset_id in reversed(self.state.render_order_2d):
            ...
            if (
                ...
                continue
            x, y = (
                numeric_series(frame, dataset.x2d),
                numeric_series(frame, dataset.y2d),
            )
            errors = {
                str(column): numeric_series(frame, str(column))
                for column in frame.columns
            }
            output.append(
                {
                    "key": dataset.dataset_id,
                    "label": dataset_display_name(dataset),
                    "x": x,
                    "y": y,
                    "errors": errors,
                }
    ...
    def _apply_canvas_size(self):
        width_in, height_in = (
            self.figure_config.width_cm / 2.54,
            self.figure_config.height_cm / 2.54,
        )
        self.figure.set_size_inches(width_in, height_in, forward=False)
        screen = self.screen()
        dpi = screen.logicalDotsPerInch() if screen else 96
        self.canvas.setFixedSize(
            max(100, round(width_in * dpi)), max(100, round(height_in * dpi))
        )
    ...
    def open_2d_property(self):
        ...
        draft.curves = [
            curves[value]
            for value in self.state.render_order_2d
            if value in available_ids and value in curves
        ]
        names = {d.dataset_id: dataset_display_name(d) for d in available}
        columns = {
            d.dataset_id: [str(c) for c in self.frames[d.dataset_id].columns]
            for d in available
        }
        initial = (
            self.state.active_dataset_id
            if self.state.active_dataset_id in available_ids
            else (draft.curves[0].column if draft.curves else "")
        )
        dialog = None
        ...
        def apply_config(config):
            ...
            self._apply_2d_config(
                config, active, available_ids, dialog.format_templates
            )
        ...
        dialog = PlotPropertyDialog(
            ...
            [],
            apply_config,
            self._save_2d_format_file,
            self,
            shared_y_axis=True,
            curve_names=names,
            curve_columns=columns,
            initial_curve_id=initial,
            load_format_callback=self._load_2d_format_file,
            format_templates=self._curve_style_templates,
        )
        dialog.exec()

    def _apply_2d_config(
        self, config, active_id=None, edited_ids=None, format_templates=None
    ):
        ...
        config.curves = [
            edited.get(value, previous.get(value))
            for value in self.state.render_order_2d
            if edited.get(value, previous.get(value)) is not None
        ]
        self.figure_config = config
        iterator = iter(edited_order)
        self.state.render_order_2d = [
            next(iterator) if value in edited_ids else value
            for value in self.state.render_order_2d
        ]
        ...
        self.figure_config.curves = [
            curve_map[value]
            for value in self.state.render_order_2d
            if value in curve_map
        ]
        if active_id:
            self._activate_dataset(active_id)
        template_source = [
            edited[value] for value in edited_order if value in edited
        ] or list(format_templates or self._curve_style_templates)
    ...
    def _save_2d_format_file(self, config, format_templates):
        ...
        if not path:
            return
        if not path.lower().endswith(".mindes2dstyle.json"):
            path += ".mindes2dstyle.json"
        templates = config.curves or format_templates or self._curve_style_templates
        try:
            Path(path).write_text(
                json.dumps(
                    make_2d_style_payload(config, templates),
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
    ...
    def _load_2d_format_file(self, current, _format_templates):
        ...
        if not path:
            return None
        try:
            style, templates = parse_2d_style_payload(
    ...
    def _configured_3d(self):
        ...
        for dataset_id in reversed(self.state.render_order_3d):
            ...
            if (
                not dataset.enabled
                or frame is None
                or not all((dataset.x3d, dataset.y3d, dataset.z3d))
            ):
                continue
            items.append(
                (
                    dataset,
                    numeric_series(frame, dataset.x3d),
                    numeric_series(frame, dataset.y3d),
                    numeric_series(frame, dataset.z3d),
                )
            )
        return items
    ...
    def draw_3d(self):
        ...
        raw_max = np.nanmax(union, axis=0)
        if not cfg.auto_bounds:
            raw_min = np.array([cfg.x_min, cfg.y_min, cfg.z_min])
            raw_max = np.array([cfg.x_max, cfg.y_max, cfg.z_max])
        ...
        for dataset, x, y, z in items:
            if not cfg.auto_bounds:
                ...
                x = np.where(in_range, x, np.nan)
                y = np.where(in_range, y, np.nan)
                z = np.where(in_range, z, np.nan)
            mode = dataset.mode3d
            if mode == "Scatter":
                poly = build_scatter(x, y, z, transform)
            else:
                result = build_surface_with_holes(x, y, z, transform)
                if result.polydata is None:
                    poly = build_scatter(x, y, z, transform)
                    fallback_messages.append(
                        f"{dataset.label}: {result.reason}; Scatter used"
                    )
                    mode = "Scatter"
                else:
                    poly = result.polydata
            mapper = vtk.vtkPolyDataMapper()
            mapper.SetInputData(poly)
            valid_z = z[np.isfinite(z)]
            if dataset.color_mode == "Z Colormap" and len(valid_z):
                value_range = (
                    (float(np.min(valid_z)), float(np.max(valid_z)))
                    if dataset.auto_color_range
                    else (dataset.color_min, dataset.color_max)
                )
                lut = make_lookup_table(dataset.colormap, value_range)
                mapper.SetLookupTable(lut)
                mapper.SetScalarRange(*value_range)
                mapper.ScalarVisibilityOn()
                lut_by_id[dataset.dataset_id] = lut
            else:
                mapper.ScalarVisibilityOff()
            actor = vtk.vtkActor()
            actor.SetMapper(mapper)
            actor.GetProperty().SetOpacity(dataset.opacity)
            actor.GetProperty().SetColor(*hex_to_rgb(dataset.color))
            if mode == "Scatter":
                actor.GetProperty().SetRepresentationToPoints()
                actor.GetProperty().SetPointSize(dataset.point_size)
            elif mode == "Mesh":
                actor.GetProperty().EdgeVisibilityOn()
                actor.GetProperty().SetEdgeColor(*hex_to_rgb(dataset.mesh_color))
                actor.GetProperty().SetLineWidth(dataset.mesh_width)
            self.renderer.AddActor(actor)
            actor_entries.append((dataset, poly))
        display_max = (raw_max - raw_min) * scale
        self._add_axes(
            (0, display_max[0], 0, display_max[1], 0, display_max[2]), raw_min, raw_max
        )
        self._add_active_scalarbar(lut_by_id)
        self._add_legend(actor_entries)
        self.renderer.SetBackground(*self._background_rgb(cfg.background))
    ...
    def _add_axes(self, bounds, raw_min, raw_max):
        ...
        if not cfg.show_axes:
            self.axes_bundle = None
            return
        self.axes_bundle = build_cube_axes_bundle(
            cfg, bounds, raw_min, raw_max, self.renderer.GetActiveCamera()
        )
        self.axes = self.axes_bundle.base
        for actor in self.axes_bundle.actors:
            self.renderer.AddActor(actor)

    def _add_active_scalarbar(self, luts):
        if not self.vtk_config.show_colorbar or not luts:
            return
        active = self._selected_dataset()
        dataset_id = (
            active.dataset_id
            if active and active.dataset_id in luts
            else next(
                (value for value in self.state.render_order_3d if value in luts),
                next(iter(luts)),
            )
        )
        dataset = next(d for d in self.state.datasets if d.dataset_id == dataset_id)
        self.scalarbar.SetLookupTable(luts[dataset_id])
        self.scalarbar.SetTitle(dataset.z3d or dataset.label)
        color = hex_to_rgb(self.vtk_config.x_axis.label_style.color)
        self.scalarbar.GetTitleTextProperty().SetColor(*color)
        self.scalarbar.GetLabelTextProperty().SetColor(*color)
        self.renderer.AddActor2D(self.scalarbar)

    def _add_legend(self, entries):
        if not self.vtk_config.show_legend or not entries:
            return
        legend = vtk.vtkLegendBoxActor()
        legend.SetNumberOfEntries(len(entries))
        legend.SetPosition(0.02, 0.02)
        legend.SetWidth(0.22)
        legend.SetHeight(min(0.35, 0.06 * len(entries) + 0.05))
        ...
        for index, (dataset, _) in enumerate(entries):
            legend.SetEntry(
                ...
                hex_to_rgb(dataset.color),
            )
        legend.GetEntryTextProperty().SetColor(
            *hex_to_rgb(self.vtk_config.x_axis.label_style.color)
        )
    ...
    def open_3d_property(self):
        ...
        active_id = (
            ...
            if self.state.active_dataset_id in ids
                ...
                (value for value in self.state.render_order_3d if value in ids), ""
            )
        )
    ...
    def _apply_3d_config(
        ...
        edited = {dataset.dataset_id: dataset for dataset in datasets}
        edited_ids = set(edited)
        for index, dataset in enumerate(self.state.datasets):
            if dataset.dataset_id in edited:
                self.state.datasets[index] = edited[dataset.dataset_id]
        ...
        self.state.render_order_3d = [
            next(iterator) if value in edited_ids else value
            for value in self.state.render_order_3d
        ]
        self.state.active_dataset_id = active_id
        self._refresh_cards()
        template_source = [
            edited[value] for value in edited_order if value in edited
        ] or list(format_templates or self._dataset_style_templates)
        ...
        for index, dataset_id in enumerate(value for value in order if value in by_id):
            if index < len(templates):
                by_id[dataset_id] = apply_dataset_template(
                    by_id[dataset_id], templates[index]
                )
        return (
            apply_3d_visual_style(current, style),
            [by_id.get(dataset.dataset_id, dataset) for dataset in datasets],
            templates,
        )
